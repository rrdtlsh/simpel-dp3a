...
class EvaluasiPugController extends Controller
{
    ...
    public function tambahPertanyaan(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'kode'            => 'required|string|max:20|regex:/^[a-zA-Z0-9.]+$/',
            'pertanyaan'      => 'required|string|max:50',
            'skor_maksimal'   => 'required|numeric|min:0|max:100',
            'pilihan_jawaban' => 'required|array|min:1',
            'petunjuk'        => 'nullable|string|max:500',
        ], [
            'kode.regex' => 'Kode hanya boleh berisi huruf, angka, dan titik (tanpa spasi atau simbol lain).',
        ]);
    ...
    public function exportExcel(Request $request)
    {
        ...
        $fileName = 'hasil-input-evaluasi-mandiri-' . $tahun . '.csv';
        $headers  = [
            'Content-Type'        => 'text/csv; charset=UTF-8',
            'Content-Disposition' => 'attachment; filename="' . $fileName . '"',
        ];

        $callback = function () use ($komponen, $tahun) {
            $handle = fopen('php://output', 'w');
            fputs($handle, chr(0xEF) . chr(0xBB) . chr(0xBF)); // BOM UTF-8

            fputcsv($handle, ['Kode', 'Komponen', 'Indikator', 'Pertanyaan', 'Jawaban', 'Skor', 'Skor Maks', 'Status', 'Catatan', 'Diisi Oleh', 'Tanggal Isi', 'Lampiran']);
