    ...
    function checkEditorValidation() {
        ...
        if (!isValid) {
            if (editor) {
                ...
                editor.style.boxShadow = '0 0 0 2px rgba(239, 68, 68, 0.2)';
            }
            if (counterAdmin) counterAdmin.style.color = '#ef4444';
            if (errEditor) {
                errEditor.querySelector('span').innerText = 'Catatan melebihi batas 500 karakter.';
                errEditor.style.display = 'flex';
            }
        } else {
            if (editor) {
            ...
            if (counterAdmin) counterAdmin.style.color = '#6b7280';
...
function initDokumenMasukPage() {
    ...
    function applyFilter() {
        ...
        document.querySelectorAll('#dokumenTable tbody .row-data').forEach(function(row) {
            ...
            // Baris akan tampil jika Lolos Filter Bidang, Tahun, DAN Pencarian Teks
...
function loadPage(page, element) {
    ...
    if (page === 'dashboard') { window.location.href = '/admin/dashboard'; return Promise.resolve(); }
...
/*
 * ✅ FIX BUG 1: handleNotifClick menggunakan getCsrf()
 */
...
/* ═══════════════════════════════════════════════════════════════════════════
   5. DOM LOADED & EVENT DELEGATION (100% SPA Safe)
   ═══════════════════════════════════════════════════════════════════════════ */
document.addEventListener('DOMContentLoaded', function() {

    /* --- CHART RENDER (Hanya untuk Dashboard) --- */
    if (document.getElementById('adminChartBar') && document.getElementById('adminChartDonut')) {
        var adminStats = {
        ...
        new ApexCharts(document.getElementById('adminChartBar'), {
        ...
        new ApexCharts(document.getElementById('adminChartDonut'), {
...
/* --- EVENT DELEGATION GLOBAL (GLOBAL CLICKS) --- */
/* Karena SPA, kita delegasikan event dari document.body agar tidak pernah putus */
