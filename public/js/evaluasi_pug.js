var EvaluasiPUG = (function () {
    ...
    /* ── Helpers ────────────────────────────────────────────────────── */
    function getCsrf() {
        return _cfg.csrf || document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';
    }
    ...
    function bindTreeToggle(container) {
        ...
        /* Indikator toggle */
        container.querySelectorAll('.epug-indikator-header').forEach(function (header) {
    ...
    function bindTabs(container) {
        container.querySelectorAll('.epug-tab').forEach(function (tab) {
            if (tab.dataset.epugBound) return;
            tab.dataset.epugBound = '1';
            tab.addEventListener('click', function () {
                container.querySelectorAll('.epug-tab').forEach(function (t) { t.classList.remove('active'); });
                container.querySelectorAll('.epug-tab-content').forEach(function (c) { c.style.display = 'none'; });
                this.classList.add('active');
                var tabId = 'epugTab' + this.dataset.tab.charAt(0).toUpperCase() + this.dataset.tab.slice(1);
    ...
    function renderPilihan(pilihan, jwb) {
        ...
        pilihan.forEach(function (item) {
            var isSelected = jwb && jwb.jawaban_kode && jwb.jawaban_kode.startsWith(item.kode || item.label.substr(0, 3));

            var div = document.createElement('div');
            div.className = 'epug-pilihan-option' + (isSelected ? ' selected' : '');
            div.setAttribute('data-kode', item.kode || '');
            div.setAttribute('data-label', item.label || '');
            div.setAttribute('data-skor', item.skor || 0);

            div.innerHTML =
                '<input type="radio" name="epugPilihan" value="' + escHtml(item.kode || '') + '"'
                + (isSelected ? ' checked' : '')
                + (isLocked ? ' disabled' : '') + '>'
    ...
    function onPilihanClick(div, item) {
        /* Deselect semua */
        div.closest('.epug-pilihan-wrap').querySelectorAll('.epug-pilihan-option').forEach(function (d) {
    ...
    function renderAuditLog(logs, versi) {
        ...
        if (!logs.length && !versi.length) {
            auditList.innerHTML = '<p style="color:#9ca3af;font-size:.8rem;padding:8px;">Belum ada riwayat.</p>';
            return;
        }
        ...
        logs.forEach(function (log) {
            ...
            div.innerHTML =
                '<div class="epug-audit-aksi">' + (aksiLabel[log.aksi] || log.aksi) + '</div>'
                + '<div class="epug-audit-meta">'
                + escHtml(log.user?.name || 'User') + ' &bull; ' + fmtTanggal(log.created_at)
                + '</div>';
        ...
        if (versi.length > 1) {
            ...
            versi.forEach(function (v) {
                ...
                vDiv.innerHTML =
                    '<div class="epug-audit-aksi">Versi ' + v.versi + ' — ' + escHtml(v.jawaban_label || '-') + '</div>'
                    + '<div class="epug-audit-meta">Skor: ' + v.skor + ' &bull; ' + escHtml(v.user?.name || '-') + ' &bull; ' + fmtTanggal(v.created_at) + '</div>';
    ...
    function updateCatatanCounter() {
        ...
        showFieldError('epugErrCatatan', isOver ? 'Catatan melebihi batas 1000 karakter.' : '');
    }
    ...
    function validateAndSave() {
        ...
        if (catatan.length > 1000) {
            ...
            showFieldError('epugErrCatatan', 'Catatan mengandung karakter yang tidak diizinkan.');
            hasError = true;
        }
        ...
        apiFetch(_cfg.routes.simpan, {
            ...
            .then(function (res) {
                if (res.ok && res.data.success) {
                    ...
                    Object.keys(errs).forEach(function (field) {
                        showFieldError('epugErr' + field.charAt(0).toUpperCase() + field.slice(1), errs[field][0]);
                    });
                } else {
            ...
            .finally(function () {
                if (simpanBtn) { simpanBtn.disabled = false; simpanBtn.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> Simpan Jawaban'; }
            });
    }
    ...
    function handleFileUpload(files) {
        ...
        if (files.length > 10) { showFieldError('epugErrFile', 'Maksimal pilih 10 file sekaligus.'); return; }
        ...
        if (zoneSpan) zoneSpan.textContent = "Mengunggah " + files.length + " file...";
            ...
            .then(function (res) {
                if (res.ok && res.data.success) {
                    ...
                    res.data.lampirans.forEach(function (lmp) {
                        ...
                        newItem.innerHTML = '<span><i class="fa-solid fa-file" style="margin-right:5px;color:#067fb2;"></i>' + escHtml(lmp.nama_file) + '</span>'
                            + '<div style="display:flex;align-items:center;gap:8px;"><a href="' + escHtml(lmp.url) + '" target="_blank" download><i class="fa-solid fa-download"></i></a>'
                            + '<button class="epug-lampiran-del" data-lampiran-id="' + lmp.id + '"><i class="fa-solid fa-trash-can"></i></button></div>';
                        list.appendChild(newItem);
                    });

                    if (zoneSpan) zoneSpan.textContent = "Klik untuk upload (PDF, DOC, XLS, IMG — maks 10 MB)";
                } else {
                    showFieldError('epugErrFile', res.data.message || 'Upload gagal.');
                    if (zoneSpan) zoneSpan.textContent = "Klik untuk upload (PDF, DOC, XLS, IMG — maks 10 MB)";
                }
            })
            .catch(function () {
    ...
    function refreshRow(pertanyaanId) {
        ...
        apiFetch(_cfg.routes.show + pertanyaanId + '?tahun=' + _state.tahun)
            ...
            .then(function (data) {
                ...
                var statusClass = { disetujui: 'epug-status-green', diisi: 'epug-status-blue', ditolak: 'epug-status-red', belum: 'epug-status-red' };
                var statusLabel = { disetujui: 'Disetujui', diisi: 'Sudah Diisi', ditolak: 'Ditolak', belum: 'Belum Diisi' };

                if (badge) {
                    badge.className = 'epug-status-badge ' + (statusClass[status] || 'epug-status-red');
                    badge.textContent = statusLabel[status] || 'Belum Diisi';
                }
                if (skorEl && jwb) {
                    skorEl.textContent = 'Skor: ' + jwb.skor + ' / ' + (row.dataset.skorMaks || '-');
                }
    ...
    function bindEvents() {
        ...
        mc.addEventListener('click', function (e) {
            ...
            if (e.target.closest('.epug-lampiran-del')) {
                ...
                Swal.fire({
                    title: 'Hapus lampiran?', icon: 'warning', showCancelButton: true,
                    ...
                    apiFetch(_cfg.routes.hapusFile + lid, { method: 'DELETE' })
                        ...
                        .then(function (d) {
                            if (d.success) btnDel.closest('.epug-lampiran-item').remove();
                            else Swal.fire('Gagal', 'Tidak dapat menghapus file.', 'error');
                        });
                });
                return;
            }
    ...
    function bindExport(container) {
        container.querySelector('#epugBtnExportExcel')?.addEventListener('click', function () {
            var tahun = document.getElementById('epugTahunSelect')?.value || new Date().getFullYear();
            Swal.fire({ title: 'Menyiapkan Excel...', allowOutsideClick: false, didOpen: function () { Swal.showLoading(); } });
            window.location.href = _cfg.routes.exportExcel + '?tahun=' + tahun;
            setTimeout(function () { Swal.close(); }, 2000);
        });

        container.querySelector('#epugBtnExportPdf')?.addEventListener('click', function () {
            var tahun = document.getElementById('epugTahunSelect')?.value || new Date().getFullYear();
            Swal.fire({ title: 'Menyiapkan PDF...', allowOutsideClick: false, didOpen: function () { Swal.showLoading(); } });
            window.location.href = _cfg.routes.exportPdf + '?tahun=' + tahun;
    ...
    function renderChart() {
        ...
        try {
            ...
            var options = {
                ...
                chart: { type: 'bar', height: 230, toolbar: { show: false }, parentHeightOffset: 0 },
                colors: ['#067fb2'],
                plotOptions: {
                    bar: { borderRadius: 4, horizontal: false, columnWidth: '45%', dataLabels: { position: 'top' } }
                },
                dataLabels: {
                    enabled: true,
                ...
                xaxis: {
                    ...
                    labels: { style: { fontSize: '11px', fontFamily: 'Poppins', fontWeight: 600 } }
                },
                yaxis: {
            ...
            chart.render();
        } catch (e) { console.error("Gagal memuat grafik PUG:", e); }
    }
